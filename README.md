...
| `FORWARDED_ALLOW_IPS`       | `172.25.0.0/16` (compose) / `127.0.0.1` (uvicorn) | Peers whose `X-Forwarded-Proto`/`-For` uvicorn trusts. Compose defaults it to the `caddy_net` subnet so redirects keep the `https` scheme and logs see the real client IP; check with `docker network inspect caddy_net` if that network is recreated.                          |
